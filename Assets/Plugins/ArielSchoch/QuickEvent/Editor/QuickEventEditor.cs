...
public class QuickEventEditor : PropertyDrawer
{
    ...
    private class PersistentCallData
    {
        ...
        public PersistentCallData(InspectorData inspectorData, int index, int indentLevel, Rect position, SerializedProperty property, SerializedProperty listProperty, GenericMenu.MenuFunction2 menuSelectionHandler, GenericMenu.MenuFunction2 noFunctionHandler)
        {
            ...
            if (targetProperty.objectReferenceValue is Component)
                targetObject = (targetProperty.objectReferenceValue as Component).gameObject;
            else
                targetObject = targetProperty.objectReferenceValue;
            ...
            {
                Component[] components = (targetObject as GameObject).GetComponents(typeof(Component));
                Object[] gameObjectAndComponents = new Object[components.Length + 1];
                gameObjectAndComponents[0] = targetObject;
                ...
                // If we assigned a component to the target field, then make sure
                // that our componentIndex is still pointing to the correct component.
                // One reason this might change is because the order of components
                // may be swapped in the inspector using the move up and down options.
            ...
            {
                genericMenuData = new MemberMenuData(inspectorData, new Object[] { targetObject });
            }
            ...
            // TODO Does removing this line break anything?
    ...
    private void OnAddButton(ReorderableList list)
    {
        ...
        var addedListener = callsProperty.GetArrayElementAtIndex(callsProperty.arraySize - 1);
        // if this isn't a duplicated list element with preassigned values then set defaults
        if (addedListener.FindPropertyRelative("target").objectReferenceValue == null)
        {
            addedListener.FindPropertyRelative("genericMenuData").FindPropertyRelative("selectedComponent").intValue = -1;
            addedListener.FindPropertyRelative("isCallEnabled").boolValue = true;
        }
    ...
    private void GenericMenuSelectionHandler(object menuIndexInfo)
    {
        ...
        persistentCall.targetProperty.objectReferenceValue = (menuIndex.componentIndex < 0) ? persistentCall.targetObject : persistentCall.genericMenuData.GetComponent(menuIndex.componentIndex).component;
    ...
    private void DrawEventListener(Rect position, int index, bool isActive, bool isFocused)
    {
        ...
        // Fix for case where undo command causes mismatch between inspector event listener count
        // and actual SerializedProperty event listener count. Update inspector data to re-sync.
        if (index >= inspectorState.inspectorData.persistentCallData.Length)
        {
            inspectorState.inspectorDataIsNotNull = false;
            return;
        }
